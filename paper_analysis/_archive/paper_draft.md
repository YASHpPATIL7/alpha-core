...
limited by the sparse Bear sample (n = 121 days). We next test whether daily NSDL FII net flows Granger-cause individual factor
premia across 15 regime-factor pairs (five daily factors × three regimes),
applying Benjamini-Hochberg false discovery rate correction. No cell survives —
FII aggregate flows do not predict factor premia in any regime. A
...
*Survivorship bias.* Our 14-stock NSE universe includes only stocks with
complete return and fundamental data over 2019–2026. Firms delisted or
significantly restructured over this period — including YES Bank (capital
reconstruction) and several IL&FS subsidiaries — are excluded from historical
return calculations. This survivorship filter tends to inflate value and
profitability premia directionally; results should be interpreted accordingly.
The IIMA primary factors are survivorship-bias adjusted by construction
(Agarwalla, Jacob, and Varma, 2013), which is a principal reason we use them
...
Viterbi smoothing. The filtered forward algorithm conditions the regime
probability at time t only on observations through t, eliminating the
retrospective smoothing that contaminates states identified by Viterbi
decoding on the full sequence. Additionally, all Granger causality and
factor attribution tests condition on the prior-day regime label (t-1),
removing any simultaneity between the contemporaneous return and the state
assignment. Verification checks confirm: COVID-19 crash (March 23, 2020) is
...
**Factor series for Granger tests.** Section 4 uses IIMA's four daily factors
(MKT, SMB, HML, WML) as the primary regime-conditional premia analysis.
Section 5 Panel B uses own-constructed daily factors — MKT, SMB, HML, RMW,
and CMA — because the Granger framework requires the same daily frequency
and the IIMA library does not include profitability (RMW) or investment (CMA).
Cross-validation confirms own-constructed MKT has r = 0.907 with IIMA MKT,
so the net-flow null for the market factor is robust to factor construction.
SMB and HML Granger results from own-constructed factors should be interpreted
with the construction caveat from Section 2.2 (near-zero IIMA correlation);
we treat them as directional only and do not make strong claims from
Bear×SMB or Bear×HML cells.
...
**Methodological pre-commitments.** To address standard referee concerns
upfront: (i) HMM look-ahead bias is eliminated by filtered forward-pass
probabilities and t-1 regime conditioning; (ii) multiple testing inflation is
controlled by BH-FDR across all 15 Panel B cells; (iii) regime-conditional
factor premia (Section 4) use IIMA survivorship-adjusted factors as primary;
(iv) COVID dominance is tested by replicating all panels excluding
February–June 2020; (v) autocorrelation and heteroskedasticity in daily
returns are corrected by Newey-West HAC standard errors throughout;
(vi) mechanism claims are framed as "consistent with" rather than causal
attribution, as aggregate flow data does not support stock-level inference.
...
### 5.3 Panel B — FII Net Flow vs. Factor Premia (3 regimes × 5 own-constructed factors)
...
*Identification note.* One identification concern applies to any test that
predicts HMM-defined regime states using FII flows: the HMM uses India VIX
as an emission variable, and FII flows may be contemporaneously correlated
with VIX (as foreign investors respond to implied volatility). Predicting
HMM Bear with flows could therefore be partially circular — VIX drives Bear
labels and VIX drives FII behavior simultaneously. We address this in two
ways. First, we define Bear onset as the HMM transition filtered at t-1 (see
Section 3), so the 5-day ahead prediction window is strictly forward-looking.
Second, a robustness check using an objective drawdown criterion (Nifty 50
trailing 20-day drawdown > 8%) as the Bear onset definition yields
...
1–5 day horizon. This null is robust to the identification concern above:
even if the regime definition were partially correlated with FII behavior,
the model produces near-chance AUC, suggesting no predictive content exists.
Panel C complements Panel B: FII flows neither predict factor-level premia
(Panel B) nor signal imminent regime transitions (Panel C).
